...
class SOM_NUMPY():
    ...
    def __init__(self, neurons, dimentions, n_iter=1000, learning_rate=0.1):
        ...
        self.weights_ = None  # klusteriai
        self.labels_ = None  # paskirti įverčius

    def _assignLabels(self, samples):
        dimentions = self.weights.shape
        self.weights_ = self.weights.reshape(dimentions[0] * dimentions[1])
        labels = []
    ...
    def _updateWeights(self, sample):
        ...
        # Fund BMU neuron
        distances = cdist(self.weights.reshape(dimentions[0] * dimentions[1], dimentions[2]), sample,
                          metric='euclidean')
        ...
        influenceValues = np.exp(
            -np.multiply(distances, distances) / (2 * self.neighbourhood_radius * self.neighbourhood_radius))
    ...
    def displayClusters(self, samples):
        ...
        for i in range(self.weights.shape[0]):
            for j in range(self.weights.shape[1]):
                ...
                print("Įvertis:", cntr, "shape:", df_.shape)
                if df_.shape[0] <= 1:
                    continue
...
def main():
    ...
    import time
    starttime = time.time()
    s = SOM_NUMPY(neurons=(2, 2), dimentions=2, n_iter=500, learning_rate=0.1)
    s.train(samples)
    endtime = time.time() - starttime
    print("Užtrukta laiko: ", endtime)
    print("Klusteriai:", s.weights_)
    print("Įverčiai:", s.labels_)
